...
class TestCorpus(unittest.TestCase):
    ...
    def setUp(self):
        # Initialisation de données de test si nécessaire
        pass

    def test_add_document(self):
        # Testez la méthode add pour s'assurer qu'elle ajoute correctement un document au corpus
        corpus = Corpus("TestCorpus")
        doc = Document("Test Titre", "Test Auteur", "2022-01-09", "http://example.com", "Ceci est un test.", "Type Test")
        corpus.add(doc)

        # Assurez-vous que le document a été ajouté correctement au corpus
        self.assertEqual(len(corpus.id2doc), 1)
        self.assertEqual(len(corpus.authors), 1)

    def test_search(self):
        # Testez la méthode search pour s'assurer qu'elle trouve correctement les passages
        corpus = Corpus("TestCorpus")
        ...
        # Assurez-vous que la méthode search retourne les résultats attendus
    ...
    def test_nettoyer_texte(self):
        ...
        # Assurez-vous que le texte nettoyé correspond aux attentes
